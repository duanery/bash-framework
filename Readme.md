...
1. 可以用于并发量的控制，比如：有100个文件要下载，但只能允许10个文件同时下。
2. 多种不同的任务可以在一个协作进程池中运行。协作进程池可以看做一个命令包、命令空间、测试空间等。比如：在做某个操作的同时要监控其他参数、ping、iostat等。这些可以放到一个协作进程池中。可以统一管理、统一销毁。
3. 并行任务执行，串行任务结果。每个任务都会是不特定的时间向stdout输出结果。使用协作进程池，可以在任务执行结束时，串行化处理任务结果。比如：多个wget并行下载文件，但最终串行输出任务执行结果，wget的下载情况不会混杂在一起输出。